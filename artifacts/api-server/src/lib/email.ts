...
export function buildIcalContent(rental: {
  ...
  return [
    ...
    rental.extraDetails ? `DESCRIPTION:${rental.extraDetails.replace(/\n/g, "\\n")}` : "",
...
export function buildRentalEmailHtml(rental: {
  ...
  nights: number; totalPrice: number; rateType: string; extraDetails?: string;
  ...
  const greeting = forRenter
    ...
    : `<p style="color:#555;margin:0 0 20px">A new rental has been ${forRenter ? "confirmed" : "submitted"}.</p>`;
  return `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f9f9f9;">
  ...
  <div style="background:white;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e0e0e0;border-top:none;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
      <tr style="background:#f0f7f4;"><td style="padding:10px 12px;font-weight:bold;color:#555;width:40%;">Renter</td><td style="padding:10px 12px;">${rental.renterName}</td></tr>
      ...
      <tr><td style="padding:10px 12px;font-weight:bold;color:#555;">Check-in</td><td style="padding:10px 12px;">${rental.startDate}</td></tr>
      <tr style="background:#f0f7f4;"><td style="padding:10px 12px;font-weight:bold;color:#555;">Check-out</td><td style="padding:10px 12px;">${rental.endDate}</td></tr>
      <tr><td style="padding:10px 12px;font-weight:bold;color:#555;">Nights</td><td style="padding:10px 12px;">${rental.nights}</td></tr>
      <tr style="background:#f0f7f4;"><td style="padding:10px 12px;font-weight:bold;color:#555;">Total Price</td><td style="padding:10px 12px;font-weight:bold;color:#2d6a4f;">$${rental.totalPrice.toFixed(2)}</td></tr>
      <tr><td style="padding:10px 12px;font-weight:bold;color:#555;">Rate Type</td><td style="padding:10px 12px;">${rental.rateType === "family" ? "Family Rate" : "Standard Rate"}</td></tr>
